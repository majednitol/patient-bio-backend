...
interface ResearcherAvatarUploadProps {
  userId: string;
...
export const ResearcherAvatarUpload = ({
  userId,
  currentAvatarUrl,
  fullName,
  onUploadComplete,
}: ResearcherAvatarUploadProps) => {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_SIZE_BYTES) {
      toast({ title: "File too large", description: `Please upload an image smaller than ${MAX_SIZE_MB}MB.`, variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      const fileName = `${userId}/researcher-avatar-${Date.now()}.${fileExt}`;
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <div className="relative group">
        <Avatar
          className={cn(
            "h-24 w-24 cursor-pointer ring-2 ring-offset-2 ring-orange-500/20 transition-all",
